...
### 2026-04 — 공개 문서와 실제의 정렬 (요약)

- **Operation rules**: 공개 [RULES.yaml](../operation_rules/RULES.yaml)은 플랫폼 **v2.4.7+**와 동기된 미러이며, 티어·progress·**엔진에서 합쳐진(merge) 평면** 키(`engine_action_gate`, QKV, 줄기·`trigger_snapshot` 레인 등)를 룰 조건으로 쓸 수 있다. (내부 DB 스키마·비공개 문서 경로는 이 repo YAML 헤더·주석에 싣지 않는다.)
- **한 줄기 제품**: 상의·API·채널은 “State Engine + RULES + (선택) LLM”으로 **통합 경로를 이미 제공**한다. **“Game-like”** 비전(다중 TF·구조적 사고·최적 수)은 **단일 2026 Q2 체크리스트 완료**가 아니라, 엔진·트리거·데이터·청산·관측·UX를 **서로 다른 우선순위 큐**로 **병행**하는 내부 맨락과 **같은 방향**이다. 공개 roadmap·본 절은 **요약**만 쓴다.
- **비공개 정본**: 배포·스키마·핸드오프·티어1~3 우선순위는 비공개 모노레포의 작업 상태·**핸드오프** 문서가 SSOT(공개 repo 방문자에게 `WORK_STATUS` 경로는 노출하지 않는다).
